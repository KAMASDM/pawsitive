// src/components/Profile/components/BreedSelect.jsx
...
const DOG_BREEDS = [
  ...
  'American Bulldog', 'American Eskimo Dog', 'Australian Shepherd', 'Basenji', 'Basset Hound',
  'Beagle', 'Bernese Mountain Dog', 'Bichon Frise', 'Border Collie', 'Boston Terrier',
  'Boxer', 'Bulldog', 'Cavalier King Charles Spaniel', 'Chihuahua', 'Cocker Spaniel',
  'Dachshund', 'Dalmatian', 'Doberman Pinscher', 'French Bulldog', 'German Shepherd',
  'Golden Retriever', 'Great Dane', 'Greyhound', 'Havanese', 'Labrador Retriever',
  'Maltese', 'Miniature Schnauzer', 'Mixed Breed', 'Pomeranian', 'Poodle',
  'Pug', 'Rottweiler', 'Saint Bernard', 'Shih Tzu', 'Siberian Husky',
  'Yorkshire Terrier'
];

const CAT_BREEDS = [
  'Abyssinian', 'American Bobtail', 'American Shorthair', 'Bengal', 'Birman',
  'British Shorthair', 'Burmese', 'Devon Rex', 'Egyptian Mau', 'Exotic',
  'Himalayan', 'Maine Coon', 'Manx', 'Mixed Breed', 'Norwegian Forest',
  'Persian', 'Ragdoll', 'Russian Blue', 'Scottish Fold', 'Siamese',
  'Siberian', 'Sphynx', 'Tonkinese', 'Turkish Angora', 'Turkish Van'
];
...
const BreedSelect = ({ petType, value, onChange, otherValue, onOtherChange, error }) => {
  ...
  // Get appropriate breed list based on pet type
    ...
    <div className="mb-6">
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        <div 
          ...
          className={`w-full px-4 py-2.5 bg-white rounded-lg border ${error ? 'border-red-500' : 'border-lavender-200'} flex items-center justify-between cursor-pointer ${!petType || !['dog', 'cat'].includes(petType) ? 'opacity-60 cursor-not-allowed' : ''}`}
        >
              ...
              <div className="overflow-y-auto max-h-60 p-2">
                  ...
                  filteredBreeds.map((breed) => (
                    <motion.button
                      ...
                      className={`w-full text-left px-3 py-2.5 rounded-md text-sm transition-colors ${
                        value === breed ? 'bg-lavender-100 text-lavender-800 font-medium' : 'text-gray-700 hover:bg-lavender-50'
                      }`}
                    >
                      {breed}
                    </motion.button>
                ...
                <motion.button
                  ...
                  className={`w-full text-left px-3 py-2.5 rounded-md text-sm mt-2 border-t border-lavender-100 pt-3 ${
                    value === 'Other' ? 'bg-lavender-100 text-lavender-800 font-medium' : 'text-gray-700 hover:bg-lavender-50'
                  }`}
                >
                  Other
                </motion.button>
