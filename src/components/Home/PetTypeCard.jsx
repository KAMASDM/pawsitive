    ...
    >
      <div className="h-48 md:h-56 bg-gradient-to-br from-lavender-100 to-lavender-200 flex items-center justify-center relative overflow-hidden">
        <div
          ...
          } -bottom-6 w-32 h-32 bg-lavender-300 rounded-full opacity-40`}
        ></div>
        <div className="text-8xl">{emoji}</div>
      </div>
      <div className="p-6 border-t border-lavender-100">
        <h3 className="text-xl font-bold mb-3 text-lavender-900">
        ...
        <div className="flex justify-between items-center">
          <div className="flex space-x-2">
            {icons.map((Icon, index) => (
              <span
                key={index}
                className="h-8 w-8 rounded-full bg-lavender-100 flex items-center justify-center shadow-sm"
              >
                <Icon
                  className="text-lavender-700"
                  size={index === 2 || index === 3 ? 16 : 14}
                />
              </span>
            ))}
          </div>
