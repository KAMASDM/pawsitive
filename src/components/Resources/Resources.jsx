...
const ResourcesPage = () => {
    ...
    const [filterOpenNow, setFilterOpenNow] = useState(false); // true/false
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                ...
                (err) => {
                    ...
                    setUserLocation({ lat: 22.3072, lng: 73.1812 }); // Default to Vadodara
                    setIsLocationLoading(false);
                }
            );
        } else {
            setUserLocation({ lat: 22.3072, lng: 73.1812 }); // Default if not supported
            setIsLocationLoading(false);
        }
    }, []);
    ...
    // Memoized function to fetch place details
    const fetchPlaceDetails = useCallback((service, place) => {
        return new Promise((resolve) => {
            const detailsRequest = {
                placeId: place.place_id,
                fields: [
                    'name', 'formatted_address', 'rating', 'website',
                    'international_phone_number', 'opening_hours', 'photos',
                    'reviews', 'types', 'vicinity', 'place_id', 'geometry'
                ]
            };
            ...
            service.getDetails(detailsRequest, (placeDetails, status) => {
                if (status === window.google.maps.places.PlacesServiceStatus.OK && placeDetails) {
                    ...
                    const distance = userLocation && placeDetails.geometry?.location
                        ? getDistance(userLocation.lat, userLocation.lng, placeDetails.geometry.location.lat(), placeDetails.geometry.location.lng())
                        : null;
    ...
    const getResourceSubCategory = useCallback((resource) => {
        // A simple heuristic to determine sub-category based on keywords in name/types
    ...
    // Fetch resources when categories or location change
    useEffect(() => {
        let isMounted = true;
        const fetchAllResources = async () => {
            ...
            try {
                ...
                if (activeSubCategory === "all") {
                    keywordsToSearch = sourceSubCats.flatMap(subCat => categoryKeywords[subCat.id] || [subCat.mapsType]);
                } else {
                    const matchingSubCats = sourceSubCats.filter(sc => sc.name === activeSubCategory);
                    keywordsToSearch = matchingSubCats.flatMap(subCat => categoryKeywords[subCat.id] || [subCat.mapsType]);
                }
    ...
    // De-duplicate subcategories for display
    ...
    const processedResources = useMemo(() => {
        let filtered = resources.filter(resource => {
            ...
            const openNowMatch = !filterOpenNow || resource.isOpen === true;

            return searchTermMatch && openNowMatch;
        });
        ...
        return filtered;
    }, [resources, searchTerm, sortBy, filterOpenNow]);
    ...
    // Effect to reset state on category/filter changes
    useEffect(() => {
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [searchTerm, sortBy, filterOpenNow, activeSubCategory]);


    // Close modal on outside click
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (selectedResource && !event.target.closest('.modal-content')) {
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 w-full">
            <div className="flex items-center gap-2">
                ...
                <select
                    ...
                    onChange={(e) => {
                        setResourcesPerPage(Number(e.target.value));
                        setCurrentPage(1); // Reset to first page when changing items per page
                    }}
                    className="bg-violet-50 border border-violet-200 text-violet-700 text-sm rounded-lg px-3 py-1 focus:outline-none focus:ring-2 focus:ring-violet-400"
                >
            ...
            <div className="flex items-center gap-2">
                <button
                    ...
                    className={`p-2 rounded-lg ${currentPage === 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-violet-100 text-violet-700 hover:bg-violet-200'}`}
                >
                    <FaChevronLeft />
                </button>
                ...
                <button
                    ...
                    className={`p-2 rounded-lg ${(currentPage === totalPages || totalPages === 0) ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-violet-100 text-violet-700 hover:bg-violet-200'}`}
                >
                    <FaChevronRight />
                </button>
            </div>
            ...
            <div className="min-h-screen bg-gradient-to-br from-slate-50 to-violet-50 flex items-center justify-center">
                <div className="text-center p-8 bg-white/80 backdrop-blur-md rounded-2xl shadow-lg">
                    <div className="flex items-center justify-center">
                        ...
                        <p className="text-violet-700 font-medium">Getting your location...</p>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 to-violet-50 py-8 px-4 sm:px-6 lg:px-8">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                {/* Header */}
                <motion.div
                    className="mb-8 rounded-3xl overflow-hidden relative py-4 shadow-xl"
                ...
                >
                    ...
                    <div className="relative z-10 p-4 sm:p-6 flex flex-col justify-center gap-4 sm:gap-7">
                        {/* Mobile: Search first (order-1) */}
                        <motion.div
                            className="relative w-full order-1 sm:order-2" // Mobile first, desktop second
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0, transition: { delay: 0.3 } }}
                        >
                            <input
                                type="text"
                                placeholder={`Search ${activeMainCategory} resources...`}
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                                className="w-full pl-12 pr-4 py-3 rounded-2xl border border-white/30 bg-white/20 backdrop-blur-md text-white placeholder-white/70 focus:outline-none focus:ring-2 focus:ring-white/50 focus:bg-white/30 shadow-lg"
                            />
                            <FaSearch className="absolute left-4 top-1/2 transform -translate-y-1/2 text-white/70" />
                        </motion.div>

                        {/* Desktop: Heading and Categories first (order-2 on mobile, order-1 on desktop) */}
                        <div className="flex flex-col lg:flex-row w-full justify-between items-center gap-4 order-2 sm:order-1">
                            {/* Heading is hidden on mobile (hidden) and visible from sm screens up (sm:block) */}
                            <motion.h1
                                className="text-3xl md:text-4xl font-bold text-white capitalize text-center lg:text-left hidden sm:block"
                                initial={{ y: -20, opacity: 0 }}
                                animate={{ y: 0, opacity: 1 }}
                                transition={{ duration: 0.5 }}
                            >
                                {activeMainCategory} Resources
                            </motion.h1>
                            <motion.div
                            ...
                            >
                                {mainCategories.map(cat => (
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 flex items-center gap-2 ${activeMainCategory === cat.id
                ...
                <motion.div
                    className="mb-8 p-4 sm:p-6 bg-white/80 backdrop-blur-md rounded-2xl shadow-lg border border-violet-100"
                    initial={{ opacity: 0, y: 20 }}
                    ...
                    <div className="hidden md:block">
                        <div className="flex flex-row items-center justify-between mb-4">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <label className="flex items-center gap-2 text-sm text-violet-700 cursor-pointer">
                                    <input type="checkbox" checked={filterOpenNow} onChange={() => setFilterOpenNow(!filterOpenNow)} className="w-4 h-4 text-violet-600 bg-gray-100 border-gray-300 rounded focus:ring-violet-500" />
                                    Open Now
                                </label>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 py-2 -mx-1 w-full">
                            <button
                                ...
                                className={`flex-shrink-0 px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-300 flex items-center gap-1 ${activeSubCategory === "all"
                            ...
                            >
                                <FaPaw className="text-xs" /> All
                            </button>
                            {displayedSubCategories.map(sub => (
                                <button
                                    key={sub.id + sub.name}
                                    onClick={() => setActiveSubCategory(sub.name)}
                                    className={`flex-shrink-0 px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-300 flex items-center gap-2 ${activeSubCategory === sub.name
                                        ? `bg-gradient-to-r ${categoryColor} text-white shadow-lg transform scale-105`
                                        : 'bg-violet-100 text-violet-700 hover:bg-violet-200 shadow-md'
                                        }`}
                                >
                                    {React.cloneElement(sub.icon, { className: "text-sm" })} {sub.name}
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="md:hidden space-y-2">
                        ...
                        <div>
                            <button
                                ...
                                className="w-full flex justify-between items-center py-2 text-left text-violet-700 font-semibold"
                            >
                                <span className="flex items-center gap-2"><FaListUl /> Categories</span>
                                <motion.div animate={{ rotate: isCategoryAccordionOpen ? 180 : 0 }}>
                                    <FaChevronDown />
                                </motion.div>
                            </button>
                            <AnimatePresence>
                                    ...
                                    >
                                        <div className="pt-2 pb-2 border-t border-violet-200">
                                            <div className="flex flex-wrap gap-2 py-2 w-full">
                                                <button
                                                    onClick={() => setActiveSubCategory("all")}
                                                    className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-300 flex items-center gap-1 ${activeSubCategory === "all"
                                                        ? `bg-gradient-to-r ${categoryColor} text-white shadow-lg transform scale-105`
                                                        : 'bg-violet-100 text-violet-700 hover:bg-violet-200 shadow-md'
                                                        }`}
                                                >
                                                ...
                                                {displayedSubCategories.map(sub => (
                                                    <button
                                                        key={sub.id + sub.name}
                                                        onClick={() => setActiveSubCategory(sub.name)}
                                                        className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-300 flex items-center gap-2 ${activeSubCategory === sub.name
                                                            ? `bg-gradient-to-r ${categoryColor} text-white shadow-lg transform scale-105`
                                                            : 'bg-violet-100 text-violet-700 hover:bg-violet-200 shadow-md'
                                                            }`}
                                                    >
                                                        {React.cloneElement(sub.icon, { className: "text-xs" })} {sub.name}
                                                    </button>
                                                ))}
                                            </div>
                                        </div>
                        ...
                        <div>
                            <button
                                ...
                                className="w-full flex justify-between items-center py-2 text-left text-violet-700 font-semibold"
                            >
                                <span className="flex items-center gap-2"><FaFilter /> Filters</span>
                                <motion.div animate={{ rotate: isFilterAccordionOpen ? 180 : 0 }}>
                                    <FaChevronDown />
                                </motion.div>
                            </button>
                            <AnimatePresence>
                                    ...
                                    >
                                        <div className="pt-2 pb-2 border-t border-violet-200">
                                            <div className="flex flex-col gap-3 pt-2">
                                                ...
                                                <label className="w-full flex items-center gap-2 text-sm text-violet-700 cursor-pointer bg-violet-50 border border-violet-200 rounded-lg px-3 py-2">
                                                    <input type="checkbox" checked={filterOpenNow} onChange={() => setFilterOpenNow(!filterOpenNow)} className="w-4 h-4 text-violet-600 bg-gray-100 border-gray-300 rounded focus:ring-violet-500" />
                                                    <span>Open Now</span>
                                                </label>
                                            </div>
                ...
                {error && (
                    <div className="bg-red-50 border border-red-200 text-red-700 p-6 text-center shadow-lg rounded-2xl">
                    ...
                    <>
                        {currentResources.length > 0 ? (
                            ...
                            <motion.div className="bg-white/90 p-8 text-center shadow-lg rounded-2xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                ...
                                <button
                                    onClick={() => { setSearchTerm(""); setActiveSubCategory("all"); setSortBy("default"); setFilterOpenNow(false); }}
                                    className={`bg-gradient-to-r ${categoryColor} hover:opacity-90 text-white px-6 py-3 rounded-xl font-medium shadow-lg hover:shadow-xl transition-all`}
                                >
                                    Reset Filters
                                </button>
                            </motion.div>
